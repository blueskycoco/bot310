...
#define NDCR_RD_ID_CNT_MSK(x)	(((x) << 16) & NDCR_RD_ID_CNT)

#define NDSR_MASK		(0xfff)

#define NDCB0_CMD_TYPE_MSK(x)	(((x) << 21) & NDCB0_CMD_TYPE)

#define NDCB0_ADDR_CYC_MSK(x)	(((x) << 16) & NDCB0_ADDR_CYC)
			...
			uint16_t cmd, int column, int page_addr)
{
	...
	info->ndcb1 = 0;
	info->ndcb2 = 0;
	info->ndcb0 |= NDCB0_ADDR_CYC_MSK(info->row_addr_cycles + info->col_addr_cycles);
	...
	if (cmd == cmdset->program)
		info->ndcb0 |= NDCB0_CMD_TYPE_MSK(1) | NDCB0_AUTO_RS;
...
{
	info->ndcb0 = cmd | ((cmd & 0xff00) ? NDCB0_DBC : 0);
	info->ndcb0 |= NDCB0_CMD_TYPE_MSK(2) | NDCB0_AUTO_RS | NDCB0_ADDR_CYC_MSK(3);
...
static int prepare_other_cmd(struct pxa3xx_nand_info *info, uint16_t cmd)
{
	...
	if (cmd == cmdset->read_id) {
		info->ndcb0 |= NDCB0_CMD_TYPE_MSK(3);
		info->data_size = 8;
	} else if (cmd == cmdset->read_status) {
		info->ndcb0 |= NDCB0_CMD_TYPE_MSK(4);
		info->data_size = 8;
	} else if (cmd == cmdset->reset || cmd == cmdset->lock ||
		   cmd == cmdset->unlock) {
		info->ndcb0 |= NDCB0_CMD_TYPE_MSK(5);
	} else
		return -EINVAL;
...
static int write_cmd(struct pxa3xx_nand_info *info)
{
	...
	if (wait_for_event(info, NDSR_WRCMDREQ)) {
		printk(KERN_ERR "timed out writing command %08x\n", ndcr);
		return -ETIMEDOUT;
	}

	writel(info->ndcb0, NDCB0);
	writel(info->ndcb1, NDCB0);
	writel(info->ndcb2, NDCB0);
	return 0;
}
...
static int handle_data_pio(struct pxa3xx_nand_info *info)
{
	...
	case STATE_PIO_READING:
		...
		if (rest_data_size) {
			unsigned int rest_data = readl(NDDB);
				    ...
				    const struct pxa3xx_nand_flash *f)
{
	...
	ndcr |= NDCR_RD_ID_CNT_MSK(info->read_id_bytes);
